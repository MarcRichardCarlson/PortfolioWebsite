        ...
        <div className="flex flex-col gap-8 min-w-64 max-w-full h-fit p-0 md:p-6 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-4 text-center sm:text-left">
                {columnNames.map((colName, colIndex) => (
                    <div key={colIndex}>
                        <h3 className="text-black font-extrabold text-2xl mb-4">{colName}</h3>
                        {techStacks[colName].map((tech, index) => (
                            tech && (
                                <motion.div
                                    key={tech}
                                    className="flex flex-col gap-2 text-center sm:text-left text-neutral-800 text-lg font-inter text-xs sm:text-sm md:text-md lg:text-lg"
